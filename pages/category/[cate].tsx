...
const Category = ({ achiveList, cate, length }: InferGetStaticPropsType<typeof getStaticProps>) => {
  return (
    <Layout>
      <div css={HeaderStyle}>
        {cate}:共计{length}篇文章
...
export const getStaticProps = async ({ params }: GetStaticPropsContext<any>) => {
  const { length, cateList: achiveList }: { length: number, cateList: IAchive[] } = JSON.parse(getPostListByCate(params.cate as string))
  return {
    props: {
